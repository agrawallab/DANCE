...
fly1_columns = ['Manual_Fly1', 'DANCE_Fly1', 'CADABRA_Fly1', 'Divider_Fly1']
fly2_columns = ['Manual_Fly2', 'DANCE_Fly2', 'CADABRA_Fly2', 'Divider_Fly2']

# Combine Fly 1 and Fly 2 data for each type, with error handling for missing columns
manual_frames = pd.concat([df['Manual_Fly1'], df['Manual_Fly2']] if 'Manual_Fly2' in df.columns else df['Manual_Fly1']).dropna().values.flatten()
...
frame_rate = 30
manual_frames_in_minutes = manual_frames / (frame_rate * 60)
...
def plot_raster(ax, frames, color, label):
    for frame in frames:
        ax.plot([frame, frame], [0, 1], color=color, lw=2.5, label=label)
    ax.set_ylim(0, 1)
...
line_color = 'black'
raster_colors = {
    'Manual': (128/255, 128/255, 128/255),
    'DANCE': (249/255, 64/255, 64/255),
    'CADABRA': (0/255, 234/255, 100/255),  
    'Divider': (0/255, 200/255, 200/255)
}

# Plot each set of behavior frames in its respective subplot with labels
plot_raster(axs[0], manual_frames_in_minutes, raster_colors['Manual'], 'Manual')
...
axs[0].set_ylabel('Manual', fontsize=12, labelpad=10, rotation=0, ha='right', va='center')
...
legend_patches = [
    patches.Patch(color=raster_colors['Manual'], label='Manual'),
